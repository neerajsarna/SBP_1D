...
function [expected_rate,loc_truncate] = expected_convg_rate_temp(n_eqn,t_end,n,foldername,filename)
...
result = cell(length(files),1);
    ...
    for i = 1 : length(files)
        ...
        temp_filename = strcat(foldername,'/result_Reference_temp/',files(i).name);
...
for i = 1 : n_eqn
    ...
    % get the values of a particular moment 
    value_f = cell2mat(cellfun(@(a) a(1,i),result,'Un',0));
    value_dx_f = cell2mat(cellfun(@(a) a(2,i),result,'Un',0));
    value_dt_f = cell2mat(cellfun(@(a) a(3,i),result,'Un',0));
...
even_Order = abs([P_Even_f(1), P_Even_dx_f(1),P_Even_dt_f(1),P_Even_f(1)])';
odd_Order = abs([P_Odd_f(1), P_Odd_dx_f(1),P_Odd_dt_f(1),P_Odd_f(1)])';
